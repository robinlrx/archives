<template>
	<div class="loaderScreen" ref="loaderScreen">
		...
		<div class="logo-container" ref="logoClick">
			<img src="~/assets/images/intro_logo.png" @click="playVideo" />
		</div>
...
export default {
	...
	mounted() {
		...
		this.$refs.videoTransition.src = this.supportsHEVCAlpha()
			? 'videos/intro_transition.mov'
			: 'videos/intro_transition.webm'
	...
	methods: {
		...
		start() {
			...
			document
				...
				.classList.add('button-container-transition')
			document
				.querySelector('#component--mouse')
				.classList.add('cursor-container-transition')

			this.$refs.videoTransition.addEventListener('timeupdate', () => {
				if (
					this.$refs.videoTransition.currentTime >= 1 &&
